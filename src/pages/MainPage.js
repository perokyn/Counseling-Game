...
const MainPage = (props) => {
                ...
                switch (message.data.content.playing) {
                    ...
                    case false:
                        ...
                        setTimeout(() => {
                            setOthePlayerPlaying(!othePlayerPlaying)
                            setUpdate("updated")
                            console.log('CLOSE OTHER PLayer info:', otherPlayerQ)
                        }, 1000)
                        break;
                    default:
    ...
    const handleQuestionClose_CubeVisible = () => {
        setotherPLayerQ(!otherPlayerQ)
        if (admin) {
            console.log(" ADMIN UPDATE CLOSE QUESTION REACHED", "update:", update)
            p1State.playing = false
        } else if (!admin) {
            console.log(" CLIENT UPDATE CLOSE QUESTION REACHED")
                ...
                <div style={{ position: "absolute", zIndex: 3, width: "100%", backgroundColor: "rgb(26, 213, 219)", height: "100%", opacity: 1, display: "flex", justifyItems: "center", padding: "30px", borderRadius: "10px" }}>
                    <div className=' flex flex-col h-2/6 bg-blue-500 p-8 rounded-xl opacity-100 mx-auto'>
                        {localUrl.indexOf('?') < 0 ? <div className='flex flex-col'>
                            ...
                            <textarea placeholder={airtableCode}   className=" p-2 text-sm h-10  rounded-xl mt-3 mb-3" onChange={value => setairtableCode(value)}></textarea>
